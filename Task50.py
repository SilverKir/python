...
def del_contact_item(list_contacts,contact_id,type_del=1):
        if type_del==1:
                return exec(print_menu(list_del))
        else:
                print_contact_item(list_contacts, contact_id)
...
list_del=[("           УДАЛЕНИЕ \nВы точно хотите удалить запись: ","print_contact_item(tel_contact,contact_id)"),("NO","exec(print_menu(list_edit))"),("YES","del_contact_item(tel_contact,contact_id,2)"),("Вернуться в главное меню","exec(print_menu(main_menu))" )]
